Tunnel = module("vrp", "lib/Tunnel")
Proxy = module("vrp", "lib/Proxy")

local cvRP = module("vrp", "client/vRP")
vRP = cvRP()

local pvRP = {}
pvRP.loadScript = module
Proxy.addInterface("vRP", pvRP)
local cfg = module("qz_market", "cfg/cfg")

local QZMarket = class("QZMarket", vRP.Extension)

function QZMarket:__construct()                         
    vRP.Extension.__construct(self)
end

local inMenu = false

Citizen.CreateThread(function()
    local ticks = 1000
    local inZone, shown = false, false
    while true do
        local ped = PlayerPedId()
        local pedCoords = GetEntityCoords(ped)
        for i = 1, #cfg.Markets do
            local coordo = cfg.Markets[i].coords
            if #(coordo - pedCoords) <=5 then
                ticks = 0
                if cfg.Setari.Marker then
                    r,g,b,a=table.unpack(cfg.Setari.Culori)
                    DrawMarker(21, coordo, 0,0,0, 0,0,0, 0.6,0.6,0.6, r,g,b,a, 1,0,0,1)
                end
                if #(coordo - pedCoords) <=2 then
                    inZone = true
                    if IsControlJustPressed(0,38) then
                        openMarket()
                    end
                end
            end
        end
        if cfg.Setari.TextUI then
            if inZone and not shown and not inMenu then
                exports.qz_ui:ShowTextUI("Open the market")
                shown = true
            elseif not inZone and shown or inMenu then
                exports.qz_ui:HideTextUI()
                shown = false
            end
        else
            ShowHelpNotification("Press ~INPUT_CONTEXT~ to open the market.")
        end
        Wait(ticks)
        ticks = 1000
        inZone= false
    end
end)

if cfg.Setari.Blip then
    Citizen.CreateThread(function()
        for i = 1, #cfg.Markets do
            local blipd = cfg.Markets[i]
            blip = AddBlipForCoord(blipd.coords)
            SetBlipSprite(blip, blipd.blipid)
            SetBlipDisplay(blip, 4)
            SetBlipScale(blip, blipd.blipscale)
            SetBlipColour(blip, blipd.blipcolor)
            SetBlipAsShortRange(blip, true)
    		BeginTextCommandSetBlipName("STRING")
            AddTextComponentString(blipd.bliptext)
            EndTextCommandSetBlipName(blip)
        end
    end)
end

function openMarket()
    inMenu = true
    SetNuiFocus(true, true)
    SendNUIMessage({action = "open"})
end

function CloseMenu()
    inMenu = false
    SetNuiFocus(false,false)
end

RegisterNUICallback("close", function()
    CloseMenu()
end)

RegisterNUICallback("purchase", function(data, cb)
    TriggerServerEvent("qz_market:purchase", data.cart)
    cb("ok")
end)

RegisterNetEvent("qz_market:purchaseResult")
AddEventHandler("qz_market:purchaseResult", function(success, message)
    if success then
        SendNUIMessage({action = "purchaseSuccess"})
    else
        SendNUIMessage({action = "purchaseFail"})
    end
end)

vRP:registerExtension(QZMarket)